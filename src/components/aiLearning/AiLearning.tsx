import { useState } from "react";
import {
  Brain,
  Plus,
  X,
  Trash2,
  Eye,
  Calendar,
  Sparkles,
  Search,
} from "lucide-react";
import {
  useCreateAiLearningMutation,
  useGetAiLearningQuery,
  useDeleteAiLearningMutation,
  useGetAiLearningByIdQuery,
} from "@/redux/aiLearning/aiLearningApi";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { toast } from "sonner";
import type { DailyPlan, LearningGuide } from "@/types/types";

const AiLearning = () => {
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [topic, setTopic] = useState("");
  const [durationDays, setDurationDays] = useState("7");
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedGuideId, setSelectedGuideId] = useState<string | null>(null);
  const [showPreviewModal, setShowPreviewModal] = useState(false);

  const { data, isLoading } = useGetAiLearningQuery();

  const [createAiLearning, { isLoading: isCreating }] =
    useCreateAiLearningMutation();
  const [deleteAiLearning, { isLoading: isDeleting }] =
    useDeleteAiLearningMutation();
  const { data: selectedGuideData, isLoading: isGuideLoading } =
    useGetAiLearningByIdQuery(selectedGuideId || "", {
      skip: !selectedGuideId,
    });

  const learningGuides = data?.data?.guides || [];
  const selectedGuide = selectedGuideData?.data;

  const handleCreateLearningGuide = async () => {
    if (!topic.trim()) {
      toast.error("Topic is required");
      return;
    }
    const days = parseInt(durationDays);
    if (isNaN(days) || days < 1 || days > 7) {
      toast.error("Duration must be between 1 and 7 days");
      return;
    }

    try {
      await createAiLearning({ topic, durationDays: days }).unwrap();
      toast.success("Learning roadmap created successfully");
      setShowCreateModal(false);
      setTopic("");
      setDurationDays("7");
    } catch (error: any) {
      const message =
        error?.status === 429
          ? "Rate limit exceeded. Please check your API plan or try again later."
          : "Failed to create learning roadmap";
      toast.error(message);
      console.error("Create learning guide error:", error);
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this learning guide?"))
      return;
    try {
      await deleteAiLearning({ id }).unwrap();
      toast.success("Learning guide deleted successfully");
    } catch (error: any) {
      toast.error("Failed to delete learning guide");
      console.error("Delete learning guide error:", error);
    }
  };

  const filteredGuides = learningGuides.filter((guide: LearningGuide) =>
    guide.topic.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const formatDate = (dateInput: Date | string) => {
    const date =
      typeof dateInput === "string" ? new Date(dateInput) : dateInput;
    return date.toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
      hour: "2-digit",
      minute: "2-digit",
    });
  };

  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center h-64">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 dark:border-blue-500"></div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-4 sm:p-6 transition-all duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-xl bg-gradient-to-br from-blue-500/10 to-purple-600/10 dark:from-purple-500/10 dark:to-pink-600/10 border border-blue-500/20 dark:border-purple-500/20">
              <Brain className="h-6 w-6 text-blue-500 dark:text-purple-500" />
            </div>
            <h1 className="text-xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100">
              AI Learning Roadmaps
            </h1>
            <div className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-500/10 to-purple-600/10 dark:from-purple-500/10 dark:to-pink-600/10 px-3 py-1 md:text-sm text-[0.7rem] font-medium text-blue-600 dark:text-purple-500 border border-blue-500/20 dark:border-purple-500/20">
              <Sparkles className="h-3 w-3" />
              AI-Powered Learning
            </div>
          </div>
          <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base">
            Create personalized AI-generated learning roadmaps for any topic
          </p>
        </div>

        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 dark:text-slate-300 h-4 w-4" />
            <Input
              type="text"
              placeholder="Search learning guides..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-purple-500 backdrop-blur-sm"
            />
          </div>
          <Button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center gap-2 px-4 sm:px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 dark:from-purple-500 dark:to-pink-600 text-white rounded-xl hover:from-blue-500/80 hover:to-purple-600/80 dark:hover:from-purple-500/80 dark:hover:to-pink-600/80"
            disabled={isCreating || isDeleting}
          >
            {isCreating ? (
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
            ) : (
              <>
                <Plus className="h-4 w-4" />
                Create Roadmap
              </>
            )}
          </Button>
        </div>

        {/* Learning Guides Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredGuides.map((guide: LearningGuide) => (
            <div
              key={guide._id}
              className="group bg-white/80 dark:bg-slate-800/80 rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-102"
            >
              <div className="p-4 sm:p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <Brain className="h-5 w-5 text-blue-500 dark:text-purple-500" />
                    <h3 className="font-semibold text-slate-800 dark:text-slate-100  text-sm  capitalize">
                      {guide.topic}
                    </h3>
                  </div>
                  <div className="flex items-center gap-1 ">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => {
                        setSelectedGuideId(guide._id);
                        setShowPreviewModal(true);
                      }}
                      className="text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                      disabled={isDeleting}
                    >
                      <Eye className="h-4 w-4" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => handleDelete(guide._id)}
                      className="text-red-600 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-red-600"></div>
                      ) : (
                        <Trash2 className="h-4 w-4" />
                      )}
                    </Button>
                  </div>
                </div>
                <div className="mb-4">
                  <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-3">
                    {guide.dailyPlan[0]?.description || "Learning roadmap"}
                  </p>
                </div>
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                  <div className="flex items-center gap-1">
                    <Calendar className="h-3 w-3" />
                    {formatDate(guide.createdAt)}
                  </div>
                  <div className="flex items-center gap-1">
                    <Brain className="h-3 w-3" />
                    {guide.durationDays} Days
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>

        {filteredGuides.length === 0 && (
          <div className="text-center py-12">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 mb-4">
              <Brain className="h-8 w-8 text-slate-500 dark:text-slate-400" />
            </div>
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">
              No learning guides found
            </h3>
            <p className="text-slate-600 dark:text-slate-300 mb-4 text-sm sm:text-base">
              {searchTerm
                ? "Try adjusting your search terms"
                : "Create your first learning roadmap to get started"}
            </p>
            <Button
              onClick={() => setShowCreateModal(true)}
              className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 dark:from-purple-500 dark:to-pink-600 text-white rounded-lg hover:from-blue-500/80 hover:to-purple-600/80 dark:hover:from-purple-500/80 dark:hover:to-pink-600/80"
              disabled={isCreating || isDeleting}
            >
              {isCreating ? (
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
              ) : (
                <>
                  <Plus className="h-4 w-4" />
                  Create Roadmap
                </>
              )}
            </Button>
          </div>
        )}

        {showCreateModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full">
              <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700">
                <h2 className="text-lg sm:text-xl font-semibold text-slate-800 dark:text-slate-100">
                  Create Learning Roadmap
                </h2>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setShowCreateModal(false)}
                  className="text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                  disabled={isCreating}
                >
                  <X className="h-4 w-4 sm:h-5 sm:w-5" />
                </Button>
              </div>
              <div className="p-4 sm:p-6">
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
                      Topic
                    </label>
                    <Input
                      type="text"
                      placeholder="Enter topic (e.g., Learn Python)"
                      value={topic}
                      onChange={(e) => setTopic(e.target.value)}
                      className="mt-1 w-full py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-100"
                      disabled={isCreating}
                    />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
                      Duration (1-7 days)
                    </label>
                    <Input
                      type="number"
                      min="1"
                      max="7"
                      value={durationDays}
                      onChange={(e) => setDurationDays(e.target.value)}
                      className="mt-1 w-full py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-100"
                      disabled={isCreating}
                    />
                  </div>
                  <Button
                    onClick={handleCreateLearningGuide}
                    className="w-full bg-gradient-to-r from-blue-500 to-purple-600 dark:from-purple-500 dark:to-pink-600 text-white rounded-xl hover:from-blue-500/80 hover:to-purple-600/80 dark:hover:from-purple-500/80 dark:hover:to-pink-600/80"
                    disabled={isCreating}
                  >
                    {isCreating ? (
                      <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                    ) : (
                      "Generate Roadmap"
                    )}
                  </Button>
                </div>
              </div>
            </div>
          </div>
        )}

        {showPreviewModal && selectedGuideId && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
              <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  <Brain className="h-5 w-5 text-blue-500 dark:text-purple-500" />
                  <h2 className="text-lg sm:text-xl font-semibold text-slate-800 dark:text-slate-100">
                    {selectedGuide?.topic || "Loading..."}
                  </h2>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => {
                    setShowPreviewModal(false);
                    setSelectedGuideId(null);
                  }}
                  className="text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                >
                  <X className="h-4 w-4 sm:h-5 sm:w-5" />
                </Button>
              </div>
              <div className="p-4 sm:p-6 overflow-y-auto max-h-96">
                {isGuideLoading ? (
                  <div className="flex items-center justify-center h-32">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 dark:border-purple-500"></div>
                  </div>
                ) : selectedGuide ? (
                  <>
                    {selectedGuide.dailyPlan.map((plan: DailyPlan) => (
                      <div key={plan.day} className="mb-6">
                        <h3 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-2">
                          {plan.title}
                        </h3>
                        <p className="text-sm text-slate-700 dark:text-slate-300 mb-2">
                          {plan.description}
                        </p>
                        <ul className="list-disc pl-5 text-sm text-slate-600 dark:text-slate-400">
                          {plan.resources.map((resource, index) => (
                            <li key={index}>{resource}</li>
                          ))}
                        </ul>
                      </div>
                    ))}
                    <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400 pt-4 border-t border-slate-200 dark:border-slate-700">
                      <span>
                        Created: {formatDate(selectedGuide.createdAt)}
                      </span>
                      <span>{selectedGuide.durationDays} Days</span>
                    </div>
                  </>
                ) : (
                  <p className="text-sm text-slate-600 dark:text-slate-300">
                    Failed to load learning guide.
                  </p>
                )}
              </div>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default AiLearning;
